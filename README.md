...
$env:CANDIDATE_CSV="output/candidates.csv"
Rscript scripts/step1_esm_discovery_spike.R
```

The default lookback window is seven days. Change `LOOKBACK_DAYS` in the script for a different window. Set `CANDIDATE_CSV` to keep the filtered candidates for the next step. Scripts live in `scripts/`; generated CSVs are meant to go in `output/` (gitignored, not committed).
...
$env:TRIAGE_INPUT_CSV="output/candidates.csv"
$env:TRIAGE_OUTPUT_CSV="output/triage_v3.csv"
Rscript scripts/triage_candidates.R
```
...
```powershell
$env:TRIAGE_OUTPUT_CSV="output/triage_v3.csv"
$env:REPORT_OUTPUT_MD="reports/weekly_report.md"
...
- `scripts/`: the three R scripts (discovery, triage, report).
- `output/`: working CSVs generated by the scripts; gitignored, not committed.
- `reports/`: dated Markdown reports (`weekly-report-YYYY-MM-DD.md`) committed by the weekly workflow run.
...
The email address is kept as a secret even though OpenAlex only uses it for polite-pool identification. The workflow uploads the dated report as an artifact and commits it to `reports/`; it does not create issues or send email.
